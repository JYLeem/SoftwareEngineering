...
def GoUserManagePage(adminID):
    window.destroy()
    subprocess.run(['python', 'AdminUserManagePage.py',adminID])
    
def GoWordManagePage(adminID):
    window.destroy()
    subprocess.run(['python', 'AdminWordManagePage.py', adminID])
    
canvas.place(x = 0, y = 0)
adminID = sys.argv[1]
AdminIDLabel = tk.Label(window, text=adminID)
AdminIDLabel.place(x=300,y=0,width=100,height=30)
ManageWordBtn = tk.Button(text = "단어 관리", command=lambda:GoWordManagePage(adminID))
ManageWordBtn.place(x=400, y=0, width=100, height=30)

ManageUserBtn = tk.Button(text="유저 관리", command=lambda:GoUserManagePage(adminID))
